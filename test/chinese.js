...
test('use default method', (t) => {
  t.plan(1);
  const customLunr = require('../index')({
    userDict: path.join(__dirname, 'userdict.utf8')
  });
  const customResult = customLunr.tokenizer('小明硕士毕业于中国科学院计算所');

  t.deepEqual(customResult, [
    '小明',
    '硕士',
    '毕业',
    '于',
...
test('use cutall method', (t) => {
  t.plan(1);
  const customLunr = require('../index')({
    userDict: path.join(__dirname, 'userdict.utf8')
  });
  const customResult = customLunr.tokenizer(
    '小明硕士毕业于中国科学院计算所',
    'cutAll'
  );
...
test('use cutForSearch method', (t) => {
  t.plan(1);
  const customLunr = require('../index')({
    userDict: path.join(__dirname, 'userdict.utf8')
  });
